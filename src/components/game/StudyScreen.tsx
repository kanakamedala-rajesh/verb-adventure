      ...
      <div className="flex-1 flex flex-col z-10 py-2 sm:py-0">
        ...
        <header className="flex items-center justify-between mb-4 sm:mb-8 lg:mb-12 shrink-0 bg-white/80 p-3 sm:p-4 rounded-2xl sm:rounded-3xl border-4 border-green-100 shadow-sm md:mx-0">
          ...
          <div className="bg-yellow-100 px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl sm:rounded-2xl flex items-center gap-1.5 sm:gap-2 text-yellow-700 font-bold text-xs sm:text-sm shadow-sm border-2 border-yellow-200/50">
            <Star fill="currentColor" className="w-4 h-4 sm:w-[18px] sm:h-[18px]" /> {unlockedCount}{' '}
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-4 sm:gap-8 lg:gap-12 min-h-0 perspective-2000 py-2">
          ...
          >
            ...
            >
              ...
              >
                <div className="absolute top-4 left-4 text-blue-50 opacity-20 sm:top-8 sm:left-8 pointer-events-none">
                  <Star fill="currentColor" className="w-[60px] h-[60px] sm:w-20 sm:h-20" />
                </div>
                <div className="absolute bottom-4 right-4 text-blue-50 opacity-20 sm:bottom-8 sm:right-8 pointer-events-none">
                  <Sparkles fill="currentColor" className="w-[60px] h-[60px] sm:w-20 sm:h-20" />
                </div>

                <span className="text-blue-400 font-heading text-base sm:text-lg uppercase tracking-widest mb-2 sm:mb-4">
                  The Word:
                </span>
                <div className="flex items-center gap-3">
                  <h1 className="text-5xl sm:text-7xl md:text-8xl font-heading text-blue-600 drop-shadow-sm mb-0">
                    {currentVerb.base}
                  </h1>
                  <button
                    onClick={(e) => {
                ...
                <div
                  className="bg-blue-50 text-blue-500 px-4 py-2 sm:px-6 sm:py-3 rounded-xl sm:rounded-2xl font-bold text-sm sm:text-base flex items-center gap-2 animate-bounce mt-4 sm:mt-8"
                  aria-hidden="true"
                >
                  TAP TO FLIP! 🔄
                </div>
              ...
              >
                <div className="space-y-6 sm:space-y-12 w-full">
                  <div>
                    <span className="text-orange-400 font-heading text-xs sm:text-sm uppercase tracking-widest block mb-1 sm:mb-2">
                      Past Simple:
                    </span>
                    <div className="flex items-center justify-center gap-3">
                      <h2 className="text-3xl sm:text-5xl md:text-6xl font-heading text-orange-600">
                        {currentVerb.simple}
                      </h2>
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          speak(currentVerb.simple);
                        }}
                        disabled={isSpeaking}
                        className={`p-2 rounded-full transition-all
                  ...
                  <motion.button
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter' || e.key === ' ') handleGetExamples();
                    }}
          ...
          <div className="flex items-center gap-4 sm:gap-8 w-full shrink-0 max-w-[260px] sm:max-w-md mb-4 sm:mb-0">
            ...
            <div
              className="font-heading text-lg sm:text-xl text-slate-400 min-w-[60px] sm:min-w-[80px] text-center"
              aria-live="polite"
            >
              {currentIndex + 1} <span className="text-sm opacity-40 lowercase">of</span>{' '}
