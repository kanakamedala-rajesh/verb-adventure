...
const inputBase =
  'w-full text-2xl sm:text-4xl p-4 sm:p-6 rounded-3xl border-4 outline-none transition-all font-heading text-center placeholder:text-slate-200 bg-white shadow-inner';
  ...
  return (
    ...
    >
      <div className="space-y-3 sm:space-y-4">
        <label
          htmlFor={simpleId}
          className="block font-heading text-blue-400 text-sm sm:text-lg uppercase tracking-widest text-center"
        >
          Past Simple
        </label>
        <div className="relative group">
          <input
            ...
            className={`${inputBase} ${
              feedback === 'correct'
                ? 'border-green-400 text-green-600 bg-green-50 shadow-none'
                : feedback === 'incorrect'
                  ? 'border-red-400 text-red-600 bg-red-50 shadow-none'
        ...
        <div className="relative group">
          ...
          <AnimatePresence>
            {feedback === 'incorrect' && (
              ...
              >
                The word is:{' '}
                <span className="underline font-heading">{question.verb.participle}</span>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
    ...
    >
      ...
      >
        {question.options?.map((option: string, idx: number) => {
          ...
          let styleClass =
            'bg-white border-4 border-slate-100 text-slate-500 hover:border-blue-200 hover:text-blue-500 hover:bg-blue-50 shadow-sm';

          if (isReadOnly) {
            if (feedback) {
              if (isCorrect)
                styleClass =
                  'bg-green-50 border-green-400 text-green-600 shadow-none scale-105 z-10';
              else if (isSelected && !isCorrect)
                styleClass = 'bg-red-50 border-red-400 text-red-600 opacity-50 shadow-none';
              else
                styleClass = 'bg-slate-50 border-slate-100 text-slate-200 opacity-30 shadow-none';
            }
          } else if (isSelected) {
            styleClass =
              'bg-blue-500 border-blue-600 text-white shadow-[0_6px_0_#2563eb] -translate-y-1';
          }
  ...
  return (
    ...
    >
      <div className="text-center bg-orange-50 p-6 sm:p-8 rounded-[3rem] border-4 border-orange-100">
        <p className="text-orange-400 font-heading text-sm uppercase tracking-widest mb-4">
          Fact Check!
        </p>
        <p className="text-slate-600 font-bold text-lg sm:text-xl leading-relaxed mb-4">
          Is the{' '}
          <span className="text-orange-500 underline">
            {question.subType === 'simple' ? 'Past Simple' : 'Past Participle'}
          </span>{' '}
          of <br />
          <span className="text-3xl text-slate-800 uppercase mt-2 block font-heading tracking-widest">
            &quot;{question.verb.base}&quot;
          </span>
        </p>
        <div className="h-2 w-12 bg-orange-200 rounded-full mx-auto my-4" aria-hidden="true" />
        <p className="text-4xl sm:text-5xl font-heading text-orange-600 uppercase tracking-tight italic">
      ...
      <div className="grid grid-cols-2 gap-4 sm:gap-6" role="radiogroup" aria-label="True or False">
        {['True', 'False'].map((opt) => {
          ...
          let styleClass =
            'bg-white border-4 border-slate-100 text-slate-400 hover:border-orange-200 hover:text-orange-500 shadow-sm';

          if (isReadOnly && feedback) {
            const thisButtonIsCorrectAnswer =
              (isTrueButton && question.isTrue) || (!isTrueButton && !question.isTrue);
