    ...
    >
      ...
      >
        <div className="flex justify-between items-center mb-4 sm:mb-8 shrink-0">
          <h3
            id="palette-title"
            className="text-xl sm:text-2xl font-heading text-blue-600 uppercase flex items-center gap-2 sm:gap-3"
          >
        ...
        <div className="flex-1 overflow-y-auto grid grid-cols-4 sm:grid-cols-5 gap-2 sm:gap-4 p-1 custom-scrollbar">
          {questions.map((_, idx) => {
            let statusClass =
              'bg-white border-2 sm:border-4 border-slate-100 text-slate-300 hover:border-blue-200 hover:text-blue-400';
            let statusText = 'Ready!';

            if (mode === 'immediate') {
              if (results[idx] === true) {
                statusClass =
                  'bg-green-50 border-2 sm:border-4 border-green-400 text-green-600 font-bold shadow-sm shadow-green-100';
                statusText = 'Great Job!';
              } else if (results[idx] === false) {
                statusClass =
                  'bg-red-50 border-2 sm:border-4 border-red-400 text-red-600 font-bold shadow-sm shadow-red-100';
                statusText = 'Oops!';
              } else if (idx === currentIndex) {
                statusClass =
                  'bg-blue-500 border-2 sm:border-4 border-blue-600 text-white font-bold shadow-lg shadow-blue-200 scale-105 sm:scale-110';
              ...
              if (idx === currentIndex) {
                ...
                statusClass =
                  'bg-blue-50 border-2 sm:border-4 border-blue-200 text-blue-500 font-bold';
                statusText = 'Answered!';
              }
            }
        ...
        <div
          className="mt-6 pt-4 sm:mt-8 sm:pt-6 border-t border-slate-100 grid grid-cols-3 gap-1 text-[8px] sm:text-[10px] font-bold uppercase tracking-wider shrink-0"
          aria-hidden="true"
        >
          <div className="flex flex-col items-center gap-1 text-center">
            <span className="w-3 h-3 sm:w-4 sm:h-4 rounded-md sm:rounded-lg bg-blue-500 shadow-md"></span>{' '}
            <span className="text-slate-400">Current</span>
          </div>
          <div className="flex flex-col items-center gap-1 text-center">
            <span className="w-3 h-3 sm:w-4 sm:h-4 rounded-md sm:rounded-lg bg-green-400 shadow-md"></span>{' '}
            <span className="text-slate-400">Correct</span>
          </div>
          <div className="flex flex-col items-center gap-1 text-center">
            <span className="w-3 h-3 sm:w-4 sm:h-4 rounded-md sm:rounded-lg bg-slate-100 border-2 border-slate-200"></span>{' '}
