...
interface StartScreenProps {
  onStart: (
    ...
    count: number,
    difficulty: 'all' | 'common' | 'advanced',
  ) => void;
  onStudy: () => void;
  onSoundPop: () => void;
  stats: {
...
export const StartScreen = ({
  ...
  const [selectedDifficulty, setSelectedDifficulty] = useState<'all' | 'common' | 'advanced'>(
    'all',
  );
  ...
  return (
    <div className="flex flex-col items-center justify-center p-4 sm:p-6 text-center w-full min-h-screen relative overflow-x-hidden bg-blue-50">
      ...
      {/* Decorative Icons */}
      <div className="hidden lg:block absolute top-20 left-20 text-blue-200 animate-bounce pointer-events-none">
        <Star size={64} fill="currentColor" aria-hidden="true" />
      </div>
      <div
        className="hidden lg:block absolute bottom-20 right-20 text-orange-200 animate-bounce pointer-events-none"
        style={{ animationDelay: '1s' }}
      >
        <Sparkles size={64} fill="currentColor" aria-hidden="true" />
      </div>
      ...
      >
        <motion.div variants={itemVariants} className="flex flex-col items-center px-4">
          ...
          <p className="text-slate-500 font-body text-base sm:text-2xl font-bold tracking-wide">
            Welcome back,{' '}
            <span className="text-blue-500 underline decoration-blue-200 decoration-4 underline-offset-4">
              {userName}
            </span>
            ! ✨
          </p>
          ...
          {stats.gamesPlayed > 0 && (
            ...
            >
              <div className="bg-white/80 backdrop-blur-sm px-4 py-2 rounded-2xl border-2 border-blue-100 shadow-sm">
                <span className="text-slate-400 text-xs uppercase font-bold tracking-widest block">
                  Best Score
                </span>
            ...
            {!showExamOptions ? (
              ...
              >
                <motion.button
                  ...
                  onClick={() => {
                    onSoundPop();
                    onStart('immediate', totalVerbs, 'all');
                  }}
                ...
                >
                  <div className="flex flex-col items-start text-left">
                    <span className="text-xl sm:text-2xl font-heading uppercase tracking-wide">
                      Quick Play
                    </span>
                    <span className="text-blue-100 text-xs sm:text-sm font-bold opacity-80">
                      All {totalVerbs} verbs!
                    </span>
                  </div>
                  <Play
                    size={24}
                    fill="currentColor"
                    className="shrink-0 ml-4 sm:w-8 sm:h-8"
                    aria-hidden="true"
                  />
                </motion.button>
                ...
                >
                  <div className="flex flex-col items-start text-left">
                    ...
                    <span className="text-orange-100 text-xs sm:text-sm font-bold opacity-80">
                      Test your brain!
                    </span>
                  </div>
                  <Trophy
                ...
                >
                  <div className="flex flex-col items-start text-left">
                    ...
                    <span className="text-green-100 text-xs sm:text-sm font-bold opacity-80">
                      Study the verbs!
                    </span>
                  </div>
                  <BookOpen
              ...
              >
                <div className="text-left space-y-6">
                  <div>
                    <h3 className="text-lg sm:text-xl font-heading text-slate-700 uppercase mb-3 px-2">
                      Difficulty
                    </h3>
                    ...
                    <div className="grid grid-cols-2 gap-3 sm:gap-4">
                      {questionOptions.map((count) => (
                        <button
                          ...
                          onClick={() => {
                            onSoundPop();
                            setSelectedCount(count === 'ALL' ? 999 : (count as number));
                          }}
