...
export const QuizScreen = ({
  ...
  const feedback =
    currentResult === true ? 'correct' : currentResult === false ? 'incorrect' : null;
    ...
    <div className="w-full max-w-7xl mx-auto p-3 sm:p-6 lg:p-8 flex flex-col min-h-screen bg-blue-50 relative overflow-x-hidden">
      ...
      <header className="flex items-center justify-between mb-4 sm:mb-8 lg:mb-12 shrink-0 z-10 bg-white p-3 sm:p-4 rounded-2xl sm:rounded-[2rem] shadow-sm border-b-4 border-blue-100">
        ...
        <div className="text-center">
          ...
          <div className="text-blue-600 font-heading text-lg sm:text-2xl uppercase leading-none">
            Word {currentIndex + 1}{' '}
            <span className="text-[10px] sm:text-xs opacity-40 lowercase">of</span>{' '}
            {questions.length}
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col lg:grid lg:grid-cols-[1fr_2.5fr_1fr] gap-4 sm:gap-8 min-h-0 z-10">
        ...
        <aside className="hidden lg:flex flex-col bg-white rounded-[2.5rem] p-8 shadow-xl border-4 border-blue-50 overflow-hidden max-h-[calc(100vh-14rem)]">
          <h3 className="text-slate-400 font-heading text-sm uppercase mb-6 text-center">
            Your Journey
          </h3>
          <div className="space-y-4 overflow-y-auto pr-2 custom-scrollbar flex-1">
            {questions.map((q, idx) => (
              ...
              >
                <span
                  className={`font-heading text-sm uppercase ${idx === currentIndex ? 'text-blue-600' : 'text-slate-300'}`}
                >
                  Word {idx + 1}
                </span>
                {allResults[idx] === true ? (
                  <div className="bg-green-100 p-1.5 rounded-full">
                    <Check className="w-[14px] h-[14px] text-green-600" />
                  </div>
        ...
        <form
          ...
          className="flex-1 flex flex-col min-h-0 w-full max-w-2xl mx-auto lg:max-w-none"
        >
          {/* Progress Bar */}
          ...
          <div className="flex-1 bg-white rounded-[2.5rem] sm:rounded-[3rem] p-6 sm:p-12 shadow-2xl border-b-8 sm:border-b-[12px] border-blue-100 relative flex flex-col min-h-0 overflow-hidden">
            ...
            <div className="flex-1 flex flex-col min-h-0 overflow-y-auto custom-scrollbar pt-12 sm:pt-16 pb-4">
              <div className="text-center mb-6 sm:mb-10 shrink-0">
                <span className="uppercase font-heading text-blue-400 text-xs sm:text-base block mb-1 tracking-widest">
                  Master This Word:
                </span>
                <div className="flex items-center justify-center gap-4">
                  <h2 className="text-4xl sm:text-7xl font-heading text-slate-800 uppercase tracking-tight leading-tight">
                    {currentQ.verb.base}
                  </h2>
                  <button
                    type="button"
            ...
            <div className="flex-1">
              <motion.button
                ...
                className={`w-full h-14 sm:h-20 rounded-2xl sm:rounded-3xl transition-all flex items-center justify-center gap-2 sm:gap-4 font-heading text-sm sm:text-2xl shadow-lg sm:shadow-xl focus:outline-none focus-visible:ring-4 focus-visible:ring-blue-200
                        ${
                          mode === 'review'
                            ? 'bg-blue-500 border-b-4 sm:border-b-8 border-blue-700 text-white'
                            : mode === 'immediate' && feedback === 'correct'
                              ? 'bg-green-500 border-b-4 sm:border-b-8 border-green-700 text-white'
                              : mode === 'immediate' && feedback === 'incorrect'
                                ? 'bg-red-500 border-b-4 sm:border-b-8 border-red-700 text-white'
                                : mode === 'delayed' && currentIndex === questions.length - 1
                                  ? 'bg-orange-500 border-b-4 sm:border-b-8 border-orange-700 text-white'
        ...
        <aside className="hidden lg:flex flex-col bg-white rounded-[2.5rem] p-8 shadow-xl border-4 border-blue-50">
          <h3 className="text-slate-400 font-heading text-sm uppercase mb-8 text-center">
            Star Progress
          </h3>
